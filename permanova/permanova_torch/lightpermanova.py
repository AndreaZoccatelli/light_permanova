...
class PermAnovaSampler(LightPermAnova):
    ...
    def __init__(self, sample_1: torch.Tensor, compress: bool = False) -> None:
    ...
    def _subsample(self, share: float) -> None:
        ...
        """
        permuted_indices = torch.randperm(len(self.sample_1))
        # Select the first 'round(share * len(self.sample_1))' indices
        num_samples = torch.round(share * len(self.sample_1))
        num_samples = num_samples.to(torch.int)

        subsample_i = permuted_indices[:num_samples]
    ...
    def get_representative_sample(
        ...
        Returns:
            torch.Tensor: subsample of sample_1
        """
        for self.share in torch.arange(min_subsample_share, max_subsample_share, step):
